...
def get_holiday_data_from_geolocation(country_code, year, month, day):
    return _get_holiday_data_with_abstract_api(country_code=country_code, year=year, month=month, day=day)
...
def _validate_email_with_abstract_api(email):
    ...
    response = request_session.get(f"{url}/?api_key={email_validation_api_key}&email={email}")
...
def _get_holiday_data_with_abstract_api(country_code: str, year: int, month: int, day: int):
    ...
    response = requests.get(f"{url}/?api_key={holidays_api_key}&country={country_code}&year={year}&month={month}&day={day}")
    data = json.loads(response.content)
    if data < 1:
        return None
    data = data[0]
